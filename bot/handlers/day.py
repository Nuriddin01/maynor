...
async def ask_current_energy(message: Message) -> None:
    await message.answer(
        "Какое у вас сейчас общее состояние / ресурс по шкале 1-5?",
        reply_markup=rating_keyboard(),
    )


async def ask_current_sleepiness(message: Message) -> None:
    await message.answer(
        "Насколько вы сейчас сонны или устали по шкале 1-5?",
        reply_markup=rating_keyboard(),
    )
...
async def ask_free_time(message: Message) -> None:
    await message.answer(
        "Сколько минут свободно прямо сейчас?",
        reply_markup=shortcut_minutes_keyboard([5, 10, 15, 20, 30, 45]),
    )


async def ask_intention(message: Message) -> None:
    await message.answer(
        "Что для вас сейчас ближе?",
        reply_markup=simple_choices_keyboard(RECOVERY_LABEL, SLEEP_LABEL),
    )


async def ask_alarm_preference(message: Message) -> None:
    await message.answer(
        "Нужен ли будильник на эту сессию?",
...
async def handle_slept_last_night(message: Message, state: FSMContext) -> None:
    ...
    await ask_current_energy(message)


@router.message(DayFlow.current_energy)
async def handle_current_energy(message: Message, state: FSMContext) -> None:
    text = message.text or ""
    if is_menu_request(text):
        return await send_main_menu(message, state)
    ...
    await ask_current_sleepiness(message)


@router.message(DayFlow.current_sleepiness)
async def handle_current_sleepiness(message: Message, state: FSMContext) -> None:
    text = message.text or ""
    if is_menu_request(text):
        return await send_main_menu(message, state)
    if is_back_request(text):
        await state.set_state(DayFlow.current_energy)
        return await ask_current_energy(message)
    try:
        value = parse_scale_1_5(text)
    except ValueError:
        await message.answer(errors.INVALID_SCALE_TEXT)
...
async def handle_free_time(message: Message, state: FSMContext) -> None:
    ...
    if is_back_request(text):
        await state.set_state(DayFlow.current_sleepiness)
        return await ask_current_sleepiness(message)
    try:
    ...
    await ask_intention(message)


@router.message(DayFlow.intention)
async def handle_intention(message: Message, state: FSMContext) -> None:
    ...
    if text not in {RECOVERY_LABEL, SLEEP_LABEL}:
        await message.answer("Выберите один из вариантов кнопкой.")
        return
...
async def handle_alarm_preference(
    ...
    if is_back_request(text):
        await state.set_state(DayFlow.intention)
        return await ask_intention(message)
    try:
