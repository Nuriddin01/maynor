...
В текущей версии основной запуск не требует Docker. `python main.py` поднимает FastAPI server, SQLite-хранилище и локальный alarm worker. Domain logic не зависит от SQLite, поэтому storage можно заменить на PostgreSQL repository layer без изменения recommendation, billing и alarm use cases.
